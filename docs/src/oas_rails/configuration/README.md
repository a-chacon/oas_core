...
OasRails supports mounting the engine multiple times with different configurations. This is useful for separating documentation into distinct groups, such as a **Public API** and an **Internal API**.

This feature **does not break current implementations**. If no specific configuration is used, the engine will use the normal default settings.

### How to use it

1. **Define named configurations** in your initializer (`config/initializers/oas_rails.rb`):
...
2. **Mount the engine** in your routes (`config/routes.rb`) by passing the configuration name:

    ```ruby
    # Use 'default: { configuration: "name" }' to select the configuration
